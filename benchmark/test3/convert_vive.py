...
def convert_vive_to_relative_in_A(vive_file='vive.txt', output_file='vive_relative_in_A.txt'):
    """
    直接将vive.txt（B系TUM格式）转换为A系下、以自身初始位置为原点的相对位姿
    """
    # 绕Z轴旋转-180°的四元数
    rot_z_pi = Rotation.from_euler('z', -np.pi)
    ...
    with open(output_file, 'w') as f:
        for line in lines:
            ...
            relative_rot = Rotation.from_matrix(relative_rot_matrix)
